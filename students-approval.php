...
{
    $sid=$_GET["asid"];
    $qry="update student set approval_status=1 where s_id=".$sid;
    $res=mysqli_query($con,$qry);
}

$selquery="select s.s_id,s.s_fname,s.s_mname,s.s_lname,s.gr_no,s.en_no,s.course_id,
c.course_name,s.sem_id,sm.sem_name
from student s inner join course c on s.course_id = c.course_id
inner join semester sm on s.sem_id=sm.sem_id
where s.approval_status=0" ;
$rsstudents=mysqli_query($con,$selquery);
?>
                  ...
                  <?php
                   ...
                   else
                   {
                      ...
                      while($row=mysqli_fetch_row($rsstudents))
                      {
                          ...
                          echo "<td>".$row[7]."</td>";
                          echo "<td>".$row[9]."</td>";
                          echo "<td><a class='btn btn-primary' href='students-approval.php?asid=".$row[0]."'>Accepted</a></td>";
                          echo "<td><a class='btn btn-primary' href='students-approval.php?rsid=".$row[0]."'>Reject</a></td>";
                          echo "</tr>";
                          $cnt=$cnt+1;
